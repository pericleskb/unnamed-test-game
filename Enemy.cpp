...
double Enemy::updatePosition(sf::Time timeDelta, sf::Vector2f targetPos)
{
  sf::Vector2f enemyPos = this->GetCenterPosition();
  sf::Vector2f movementVector(targetPos.x - enemyPos.x,
...
void Enemy::damagePlayer()
{
  ...
  //we could have a protected string Enemy that will change if a fury spell is cast
